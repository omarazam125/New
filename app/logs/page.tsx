...
export default function CustomerRecordsPage() {
  ...
  async function fetchCustomerRecords() {
    try {
      ...
      console.log("[v0] Fetched reports from Supabase:", savedReports?.length || 0)
      ...
      if (response.ok) {
        ...
        const supabaseRecords = (savedReports || [])
          .map((report) => {
            ...
            return {
              ...
              time: new Date(report.created_at).toTimeString().slice(0, 5),
              duration: duration,
        ...
        setRecords([...transformedRecords, ...supabaseRecords])
        setFilteredRecords([...transformedRecords, ...supabaseRecords])
      }
    } catch (error) {
  ...
  const handleGenerateReport = async (callId: string, recordData?: CallRecord) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (saveResponse.ok) {
          console.log("[v0] Complete report saved to Supabase successfully")
        } else {
          const errorData = await saveResponse.text()
          console.error("[v0] Failed to save report to Supabase:", errorData)
        }
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[v0] Error saving notes to Supabase:", error)
      } else {
        console.log("[v0] Notes saved to Supabase successfully")
      }
